...
class BatchedAgRsPrepareAndFinalize(mk.FusedMoEPrepareAndFinalizeModular):
    ...
    Bridges the AllGather+ReduceScatter all-to-all (the only cross-rank MoE
    comm available on XPU today) with the ``[E_local, max_num_tokens, K]``
    batched format consumed by ``BatchedTritonExperts`` (the moe_mmk TD kernel).

    - ``prepare()`` all-gathers the global token batch + routing across the
      group (``get_ep_group().dispatch`` -> XCCL ``all_gatherv``), then buckets
      the gathered tokens into this rank's local expert slice.
    - ``finalize()`` reduces the batched expert output back to per-token rows
      and reduce-scatters (``combine``) so each rank gets its local slice.

    Unlike a true sparse all-to-all this replicates the full batch on every
    rank (so the batched buffer is sized to the global token count), but it
    relies only on collectives XPU already supports.
    """
